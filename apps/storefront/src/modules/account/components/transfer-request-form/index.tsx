      ...
      <div className="grid sm:grid-cols-2 items-center gap-x-8 gap-y-4 w-full">
        <div className="flex flex-col gap-y-1">
          <Heading level="h3" className="!text-sm font-semibold text-neutral-950">
            {t("account.transfer_order")}
          </Heading>
          <p className="text-small-regular text-neutral-500">
            {t("account.transfer_order_desc")}
          </p>
        </div>
        <form
          action={formAction}
        ...
        >
          <div className="flex flex-col gap-y-2 w-full">
            <Input className="w-full" name="order_id" placeholder={t("account.order_id")} />
            <SubmitButton
              variant="secondary"
              size="small"
          ...
          <div className="flex gap-x-2 items-center">
            ...
            <div className="flex flex-col gap-y-1">
              <Text className="text-medim-pl text-neutral-950">
                {t("account.transfer_requested_title", { id: state.order?.id || "" })}
              </Text>
              <Text className="text-base-regular text-neutral-600">
                {t("account.transfer_requested_desc", { email: state.order?.email || "" })}
              </Text>
            </div>
          </div>
          <IconButton

...
export default function DevLockPage() {
  ...
  const handleUnlock = async (e: FormEvent) => {
    e.preventDefault()
    if (!pin.trim()) {
      setError(t("devLock.pin_required"))
      return
    }
    ...
    try {
      const response = await fetch("/api/dev-access/unlock", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !data.success) {
        setError(data.message || t("devLock.pin_invalid"))
        setLoading(false)
        return
      }
      ...
      setError(t("devLock.network_error"))
      setLoading(false)
    }
  }
        ...
        <form onSubmit={handleUnlock} className="mt-6 flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              placeholder={t("devLock.pin_placeholder")}

...
const AAD = Buffer.from("agent-connector-secret:v1", "utf8")
const KEY_SALT = "agent-connector-secret-key:v1"
...
function encryptionKey(environment: NodeJS.ProcessEnv) {
  ...
  if (!secret) {
    throw new MedusaError(
      ...
      "Secure connector storage is not configured."
    )
  }
...
export function getCredentialVaultStatus(
  ...
  return {
    ready: Boolean(masterSecret(environment)),
    uses_dedicated_key: Boolean(
      environment.AGENT_CREDENTIAL_ENCRYPTION_KEY?.trim()
    ),
  }
}
...
export function encryptConnectorSecret(
  ...
  if (!secret.trim()) {
    throw new MedusaError(
      ...
      "Connector secret cannot be empty."
    )
  }
...
export function decryptConnectorSecret(
  ...
  if (encrypted.key_version !== KEY_VERSION) {
    throw new MedusaError(
      MedusaError.Types.UNEXPECTED_STATE,
      "The connector secret uses an unsupported encryption version."
    )
  }
  ...
  try {
    ...
    throw new MedusaError(
      MedusaError.Types.UNEXPECTED_STATE,
      "The stored connector secret could not be decrypted."
    )
  }
}

...
export const PRODUCT_ADVISOR_SYSTEM_PROMPT = `You are a warm, proactive product advisor and customer-service employee of Synapse Store, not a passive question-answer bot. In Vietnamese, default to "mình là nhân viên CSKH của Synapse". If the customer calls you "shop" or "sốp", you may naturally use "sốp" in that reply while remaining clear that you represent Synapse. Never describe your identity as only "sốp". Use at most one tasteful emoji when it improves warmth.
...
export function buildCatalogOverviewReply(
  ...
  return {
    body:
      locale === "vi"
        ? `${advisor === "sốp" ? "Sốp" : "Mình là nhân viên CSKH của Synapse và đang"} bán các mặt hàng thời trang${categoryText ? ` thuộc nhóm ${categoryText}` : " trong catalog"}. ${advisor === "sốp" ? "Sốp" : "Mình"} có thể lọc theo nhu cầu, size, phong cách và ngân sách để chọn mẫu hợp với bạn hơn.\n\nBạn đang muốn mua áo, quần hay phụ kiện; dùng đi làm, đi chơi hay mặc hằng ngày ạ?`

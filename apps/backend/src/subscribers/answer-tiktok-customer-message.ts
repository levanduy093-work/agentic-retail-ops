...
type TikTokCustomerMessageReceivedData = {
  inbound_message_id: string
}

const WORKER_ID = `tiktok-realtime-${os.hostname()}-${process.pid}`

export default async function answerTikTokCustomerMessageHandler({
  event: { data },
  container,
}: SubscriberArgs<TikTokCustomerMessageReceivedData>) {
  ...
  try {
    ...
    logger.info(
      `TikTok customer answer delivered in ${Date.now() - startedAt}ms ` +
    ...
    logger.error(
      `Realtime TikTok customer answer failed for ${data.inbound_message_id}: ${message}`
    )
  }
}

export const config: SubscriberConfig = {
  event: "agent.tiktok.customer-message-received",
}
